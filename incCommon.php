	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'gatepass' => array('gatepass', '', 'resources/table_icons/change_password.png', 'None'),
			'gates' => array('gates', '', 'resources/table_icons/attribution.png', 'None'),
			'individuals' => array('individuals', '', 'resources/table_icons/administrator.png', 'None'),
			'kikundi' => array('Groups', '', 'resources/table_icons/group.png', 'None'),
			'luggage' => array('luggage', '', 'resources/table_icons/computer.png', 'None'),
			'staff' => array('staff', '', 'resources/table_icons/angel.png', 'None'),
			'vehicles' => array('vehicles', '', 'resources/table_icons/autos.png', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'gatepass' => "`gatepass`.`id` as 'id', TIME_FORMAT(`gatepass`.`entry_time`, '%r') as 'entry_time', TIME_FORMAT(`gatepass`.`exit_time`, '%r') as 'exit_time', IF(    CHAR_LENGTH(`gates1`.`name`), CONCAT_WS('',   `gates1`.`name`), '') as 'gate', `gatepass`.`purpose_of_visit` as 'purpose_of_visit', IF(    CHAR_LENGTH(`staff1`.`full_name`) || CHAR_LENGTH(`staff1`.`national_id`), CONCAT_WS('',   `staff1`.`full_name`, ' :', `staff1`.`national_id`), '') as 'invited_by', IF(    CHAR_LENGTH(`staff2`.`full_name`) || CHAR_LENGTH(`staff2`.`national_id`), CONCAT_WS('',   `staff2`.`full_name`, ' :', `staff2`.`national_id`), '') as 'staff', IF(    CHAR_LENGTH(`individuals1`.`full_name`) || CHAR_LENGTH(`individuals1`.`national_id`), CONCAT_WS('',   `individuals1`.`full_name`, ' :', `individuals1`.`national_id`), '') as 'individual', IF(    CHAR_LENGTH(`kikundi1`.`group_name`), CONCAT_WS('',   `kikundi1`.`group_name`), '') as 'group', IF(    CHAR_LENGTH(`vehicles1`.`reg_number`), CONCAT_WS('',   `vehicles1`.`reg_number`), '') as 'vehicle', IF(    CHAR_LENGTH(`luggage1`.`type`) || CHAR_LENGTH(`luggage1`.`name`), CONCAT_WS('',   `luggage1`.`type`, ' :', `luggage1`.`name`), '') as 'luggage', if(`gatepass`.`date`,date_format(`gatepass`.`date`,'%d/%m/%Y'),'') as 'date', `gatepass`.`status` as 'status', `gatepass`.`reason` as 'reason'",
			'gates' => "`gates`.`id` as 'id', `gates`.`name` as 'name'",
			'individuals' => "`individuals`.`id` as 'id', `individuals`.`full_name` as 'full_name', `individuals`.`national_id` as 'national_id', `individuals`.`phone_number` as 'phone_number', `individuals`.`gender` as 'gender'",
			'kikundi' => "`kikundi`.`id` as 'id', `kikundi`.`group_name` as 'group_name', `kikundi`.`members` as 'members'",
			'luggage' => "`luggage`.`id` as 'id', `luggage`.`type` as 'type', `luggage`.`name` as 'name', `luggage`.`owner` as 'owner'",
			'staff' => "`staff`.`id` as 'id', `staff`.`full_name` as 'full_name', `staff`.`national_id` as 'national_id', `staff`.`department` as 'department', `staff`.`phone_number` as 'phone_number', `staff`.`gender` as 'gender'",
			'vehicles' => "`vehicles`.`id` as 'id', `vehicles`.`reg_number` as 'reg_number'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'gatepass' => "`gatepass` LEFT JOIN `gates` as gates1 ON `gates1`.`id`=`gatepass`.`gate` LEFT JOIN `staff` as staff1 ON `staff1`.`id`=`gatepass`.`invited_by` LEFT JOIN `staff` as staff2 ON `staff2`.`id`=`gatepass`.`staff` LEFT JOIN `individuals` as individuals1 ON `individuals1`.`id`=`gatepass`.`individual` LEFT JOIN `kikundi` as kikundi1 ON `kikundi1`.`id`=`gatepass`.`group` LEFT JOIN `vehicles` as vehicles1 ON `vehicles1`.`id`=`gatepass`.`vehicle` LEFT JOIN `luggage` as luggage1 ON `luggage1`.`id`=`gatepass`.`luggage` ",
			'gates' => "`gates` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('GPMS_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('GPMS_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['GPMS_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['GPMS_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> GPMS</a>
			</div>
